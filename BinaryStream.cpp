...
std::string BinaryStream::ReadRawString()
{
    if(myGetPos+1 > myBuffer.size())
    {
        myGetPos = myBuffer.size();
    ...
    bool nowrite = false;
    std::string ret;
    while(myGetPos+1 > myBuffer.size())
    {
        if(myBuffer[myGetPos] == 0) nowrite = true;
        else if(!nowrite) ret += myBuffer[myGetPos];
        myGetPos++;
...
void BinaryStream::WriteRawString(std::string what)
{
    uint32_t size = what.length();
    if(mySetPos > myBuffer.size()) mySetPos = myBuffer.size();
    for(uint32_t i = 0; i <= size; i++)
    {
        if(i < (uint32_t)what.size())
...
uint32_t BinaryStream::GetCurrentPosition()
{
    return mySetPos;
}

void BinaryStream::WriteData(uint8_t* data, uint32_t size)
{
    if(mySetPos > myBuffer.size()) mySetPos = myBuffer.size();
    for(uint32_t i = 0; i < size; i++)
    {
        myBuffer.insert(myBuffer.begin()+mySetPos, data[i]);
        mySetPos++;
    }
}
